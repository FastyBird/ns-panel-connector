...
class Execute extends Console\Command\Command
{
	...
	protected function configure(): void
	{
		$this
			->setName(self::NAME)
			->setDescription('NS Panel connector service')
			->setDefinition(
	...
	protected function execute(Input\InputInterface $input, Output\OutputInterface $output): int
	{
		...
		$io->title($this->translator->translate('//ns-panel-connector.cmd.execute.title'));

		$io->note($this->translator->translate('//ns-panel-connector.cmd.execute.subtitle'));
		...
		if (
			...
			if ($connector === null) {
				$io->warning(
					$this->translator->translate('//ns-panel-connector.cmd.execute.messages.connector.notFound'),
				);
			...
			$findConnectorsQuery->byType(Entities\NsPanelConnector::TYPE);
			...
			if (count($connectors) === 1) {
				...
				if ($input->getOption('no-interaction') === false) {
					$question = new Console\Question\ConfirmationQuestion(
						$this->translator->translate(
							'//ns-panel-connector.cmd.execute.questions.execute',
				...
				$question = new Console\Question\ChoiceQuestion(
					$this->translator->translate('//ns-panel-connector.cmd.execute.questions.select.connector'),
