...
class Execute extends Console\Command\Command
{
	...
	protected function execute(Input\InputInterface $input, Output\OutputInterface $output): int
	{
		...
		if ($input->getOption('quiet') === false) {
			$io->title((string) $this->translator->translate('//ns-panel-connector.cmd.execute.title'));

			$io->note((string) $this->translator->translate('//ns-panel-connector.cmd.execute.subtitle'));
		}
		...
		if (
			...
			if ($connector === null) {
				$io->warning(
					(string) $this->translator->translate(
						'//ns-panel-connector.cmd.execute.messages.connector.notFound',
					),
				);
			...
			if (count($connectors) === 1) {
				...
				$question = new Console\Question\ChoiceQuestion(
					(string) $this->translator->translate(
						'//ns-panel-connector.cmd.execute.questions.select.connector',
					),
		...
		if (!$connector->isEnabled()) {
			$io->warning(
				(string) $this->translator->translate('//ns-panel-connector.cmd.execute.messages.connector.disabled'),
			);
